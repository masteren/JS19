...
## 工夫した点
- タブUI — 「今日／最近／ランダム／お気に入り」を1画面で切り替え。data-tab を JS が読んで対応する取得関数を呼ぶ仕組みにした。
- API通信（api.js）と画面描画（app.js）を役割で分け、コードを読みやすくした。
- お気に入りを localStorage に保存し、リロードしても残るようにした。★ボタンの表示も状態と同期させた。
- ライトボックスに ←→・Esc・背景クリックの操作を付け、端まで行ったらループするようにした。
- 背景の星を3層に分け、スクロール速度を変えてパララックス（奥行き）を出した。
- 「今日」の画像が未公開のとき API が 500 を返す挙動に苦労したが、前日の画像に切り替えて表示する処理を入れて対応した。
